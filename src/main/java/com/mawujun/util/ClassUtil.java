...
import com.mawujun.exception.UtilException;
...
import com.mawujun.lang.Singleton;
...
public class ClassUtil {
	...
	// ----------------------------------------------------------------------------------------- Scan classes
	/**
	 * 扫描指定包路径下所有包含指定注解的类，注意是当前线程的classloaer
	 * 会扫描jar中的内容
	 * https://www.cnblogs.com/zp-uestc/p/11017708.html
	 * @param packageName 包路径,可以以分号“;”进行分隔
	...
	public static Set<Class<?>> scanPackageByAnnotation(String packageName, final Class<? extends Annotation> annotationClass) {
		...
		return scanPackageByAnnotation(packageNames,annotationClass);
	 ...
	 * @see ClassScaner#scanPackageByAnnotation(String, Class)
	 */
	public static Set<Class<?>> scanPackageByAnnotation(String[] packageNames, final Class<? extends Annotation> annotationClass) {
		Set<Class<?>> setes=new HashSet<Class<?>>();
		for(String pkg:packageNames) {
			setes.addAll( ClassScaner.scanPackageByAnnotation(pkg, annotationClass));
		}
		return setes;
	}
