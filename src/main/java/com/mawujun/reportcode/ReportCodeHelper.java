package com.mawujun.reportcode;
...
//	private static String minStr=(char)min+"";//会把数字转换成键盘上的字符
	//http://www.cnblogs.com/jeffqing/archive/2012/06/28/2568911.html 键盘对应的数字
	
	/**
	 * 产生的标准是每个节点有3位符号
	 * 10进制,4代表长度是4
	 * @param baseCode
	 * @param firstchild  true:表示获取该节点的子节点，false表示获取的是兄弟节点 
	 * @return
	 */
	public static String generate4(String baseCode,boolean firstchild){	
		if(firstchild){
			return generateFirstChildCode(baseCode,4,48,57);
		} else {
			return generate(baseCode,4,48,57);
		}
		
	}
	public static String generate3(String baseCode,boolean firstchild){	
		if(firstchild){
			return generateFirstChildCode(baseCode,3,48,57);
		} else {
			return generate(baseCode,3,48,57);
		}
	 ...
	 * @return
	 */
	public static String generateFirstChildCode(String baseCode,int len,int min,int max){	
		//return generate(baseCode+sperator+getMinCode(len),len);
		return baseCode+sperator+getMinCode(len,(char)min+"");
	}
	private static String getMinCode(int len,String minStr){
		
		return StringUtils.leftPad(minStr, len, "0");
	}
	/**
	 ...
	 * @return
	 */
	public static String generate(String baseCode,int len,int min,int max){
		String minStr=(char)min+"";
		if(baseCode==null || "".equals(baseCode.trim())){
			baseCode=getMinCode(len,minStr);
			return baseCode;
		}
		...
		return ArrayUtil.join(codes, sperator);
	}
