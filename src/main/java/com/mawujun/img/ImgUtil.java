...
import com.mawujun.io.resource.Resource;
...
public class ImgUtil {
	...
	public static void scale(Image srcImg, File destFile, float scale) throws IORuntimeException {
		Img.from(srcImg).setTargetImageType(FileUtil.getSuffixName(destFile)).scale(scale).write(destFile);
	}
	...
	public static void convert(File srcImageFile, File destImageFile) {
		...
		final String srcExtName = FileUtil.getSuffixName(srcImageFile);
		final String destExtName = FileUtil.getSuffixName(destImageFile);
	...
	public static void write(Image image, File targetFile) throws IORuntimeException {
		...
		try {
			...
			write(image, FileUtil.getSuffixName(targetFile), out);
		} finally {
			IoUtil.close(out);
		}
	}
