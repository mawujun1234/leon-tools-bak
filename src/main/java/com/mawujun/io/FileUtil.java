...
import com.mawujun.io.file.*;
import com.mawujun.io.file.FileWriter;
import com.mawujun.io.file.FileReader.ReaderHandler;
import com.mawujun.io.resource.ResourceUtil;
import com.mawujun.lang.Assert;
import com.mawujun.system.SystemUtil;
import com.mawujun.util.*;
...
public class FileUtil {
	...
	/**
	 * 把所有的路径都转换成 系统的真是路径，无论是linux还是window都统一
	 * @param path
	 * @return
	 */
	...
	/**
	 *
	 * @param packageName 包名或者类的全限定类名：com.mawujun.io或者com.mawujun.util.StringUtil
	 * @return
	 */
	public static String packageToPath(String packageName){
		return StringUtil.packageToPath(packageName);
	}
	//====================================================
	/**
	 * 统计一行多少行数
	...
	public long countLineNumber(File file) {
		return countLineNumber(file,Charset.defaultCharset().toString());
	}
	/**
	 * 统计一行多少行数
	 * @param file
	 ...
	 * @return
	 */
	public long countLineNumber(File file,String charset) {
		 if (file.exists() && file.isFile()) {
		        try {
		        	BufferedReader fileReader =getReader(file,charset);
	...
	public long countLineNumber(String filepath) {
		return countLineNumber(new File(filepath));
	}
	public long countLineNumber(String filepath,String charset) {
		return countLineNumber(new File(filepath),charset);
	}
    ...
    */
     ...
     * @param targetFileName  需要查找的文件名  例如："*.txt"
     */  
    public static List<File> listLoopFiles(String baseDirName, String targetFileName) {
    	List<File> files=new ArrayList<File>();
    	listLoopFiles(baseDirName,targetFileName,files);
    	return files;
    }
    private  static void listLoopFiles(String baseDirName, String targetFileName, List<File> fileList) {  
        /** 
         * 算法简述： 
        ...
        if (!baseDir.exists() || !baseDir.isDirectory()){  
            ...
            for (int i = 0; i < filelist.length; i++) {  
                ...
                if(!readfile.isDirectory()) {  
                    ...
                    listLoopFiles(baseDirName + File.separator + filelist[i],targetFileName,fileList);  
                }  
            }  
        }  
    } 
    
    public static List<String> listLoopFileNames(String baseDirName, String targetFileName) {
    	List<String> files=new ArrayList<String>();
    	listLoopFileNames(baseDirName,targetFileName,files);
    	return files;
    }
   
    private  static void listLoopFileNames(String baseDirName, String targetFileName, List<String> fileList) {  
        /** 
         * 算法简述： 
         * 从某个给定的需查找的文件夹出发，搜索该文件夹的所有子文件夹及文件， 
         * 若为文件，则进行匹配，匹配成功则加入结果集，若为子文件夹，则进队列。 
        ...
        if (!baseDir.exists() || !baseDir.isDirectory()){  
            ...
            for (int i = 0; i < filelist.length; i++) {  
                ...
                if(!readfile.isDirectory()) {  
                	...
                	listLoopFileNames(baseDirName + File.separator + filelist[i],targetFileName,fileList);  
                }  
            }  
        }  
    }  
...
/**
 * 获取class所在的根目录
 * 获取到的是class目录所在的位置:/Users/mawujun/git/syrs/syrs-admin/target/classes/
 * @return
 */
...
/**
 * 获取class所在的根目录，如果class在依赖jar包里面，返回null
 * 获取到的是class目录所在的位置:/Users/mawujun/git/syrs/syrs-admin/target/classes/
 * @return
 */
...
/**
 * 获取项目的绝对路径
 * 类似于：E:\xiaochegnxu\syrs\syrs-admin，syrs-admin就是项目的根目录
 * @return
 */
...
/**
 * 获取maven项目的src路径,即java类存放的路径，例如：E:\xiaochegnxu\syrs\syrs-admin\src\main\java
 * @return
 */
...
//	}
	...
	if(url.getFile().indexOf(".jar!")==-1) {
		...
		jarpath= url.getFile().substring(0, url.getFile().indexOf(".jar!")+4);
	}
	//如果是window会出现/D:/xxxxxxxxxxxxxxxx,所以要把最前面的/去掉
	if(jarpath.startsWith("/") && SystemUtil.getOsInfo().isWindows()){
...
public static void copyStream(Reader in, Writer out) throws IOException {
	 ...
	 * @return
	 */
	public static boolean isSuffix(String pathOrName,String suffix){
		//获取最后一个.的位置
		int lastIndexOf = pathOrName.lastIndexOf(".");
		if(lastIndexOf==-1){
			return false;
		}
		//获取文件的后缀名 .jpg
		//String suffix = pathOrName.substring(lastIndexOf);
		if( pathOrName.substring(lastIndexOf).equalsIgnoreCase(suffix)){
			return true;
		} else {
			return false;
		}
	 ...
	 * @since 3.2.0
	 */
	public static List<File> listLoopFiles(String path, FileFilter fileFilter) {
		return listLoopFiles(file(path), fileFilter);
	}
	 ...
	 * 如果提供file为文件，直接返回过滤结果
	 * 1:先定义过滤类
	 *     public class ControllerFilter implements FileFilter {
	 *         @Override
	 ...
	 *
	 * 2：调用
	 *  FileUtil.listLoopFiles(targetpath,new ControllerFilter());
	 * @param file 当前遍历文件或目录
	 * @param fileFilter 文件过滤规则对象，选择要保留的文件，只对文件有效，不过滤目录
	 * @return 文件列表
	 */
	public static List<File> listLoopFiles(File file, FileFilter fileFilter) {
		...
		if (file.isDirectory()) {
			...
			if (ArrayUtil.isNotEmpty(subFiles)) {
				for (File tmp : subFiles) {
					fileList.addAll(listLoopFiles(tmp, fileFilter));
				}
			}
		} else {
	 ...
	 * @since 3.2.0
	 */
	public static List<File> listLoopFiles(String path) {
		return listLoopFiles(file(path));
	}
	 ...
	 * @return 文件列表
	 */
	public static List<File> listLoopFiles(File file) {
		return listLoopFiles(file, null);
	}
	...
	public static File rename(File file, String newName, boolean isRetainExt, boolean isOverride) {
		if (isRetainExt) {
			newName = newName.concat(".").concat(FileUtil.getSuffixName(file));
		}
	 ...
	 */
	public static byte[] readBytes(InputStream is) throws IORuntimeException {
		byte[] data = null;  
	    try {  
...
//	    return b;  
	...
	public static String loadFromJar(String path){
		return null;
	}
